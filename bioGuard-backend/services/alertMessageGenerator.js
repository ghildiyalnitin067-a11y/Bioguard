 ...
 * ─────────────────────────────────────────────────────────────
 * Generates a village-friendly public message, step-by-step
 * solutions, and long-term prevention tips for every alert
 * created in the BioGuard system.
 *
 * Output is bilingual: primary message in simple English + a
 * Hindi subtitle so local communities can understand.
 */

/* ── Templates keyed by  type × severity ── */
const TEMPLATES = {
...
/**
 * generateAlertMessage(type, severity, location)
 * Returns { headline, hindi, villageMessage, solutions, prevention, whatsappText }
 */
function generateAlertMessage(type, severity, location) {
  ...
  /* WhatsApp-ready plain-text version */
  const whatsappText = [
    `${headline}`,
    `${hindi}`,
    ``,
