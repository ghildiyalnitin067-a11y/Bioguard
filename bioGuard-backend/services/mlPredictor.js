/**
 * ML Risk Predictor Service  v3 — Python-primary, JS-fallback
 * ─────────────────────────────────────────────────────────────
 * refreshAllPredictions():
 *   1. Pulls live MongoDB data (reports + incidents per region)
 *   2. Calls Python sklearn RandomForestRegressor via mlBridge
 *      for each of the 8 monitored zones (PRIMARY)
 *   3. Falls back to JS computation if Python is offline
 *
 * predictRisk() — JS-only single-point predictor (used as fallback
 * by mlBridge when Python is unavailable)
 */

/* ── mlBridge  (Python ML primary) ── */
const mlBridge = require('../ml/mlBridge');

/* ── Real-time alert feature overrides (lazy-loaded to avoid circular dep) ── */
...
/* ── Community report signals cache ── */
...
/* ── Seasonal risk multipliers for NE India ── */
const SEASON_RISK = {
...
/* ── Report type → threat type mapping ── */
...
/* ── Core prediction for one zone, given live DB data ── */
function computePrediction(zone, reportFreq, incidentSeverity, communitySignals = {}) {
  ...
  /* Top threat from zone weights, boosted by live report frequency */
  ...
  /* ── Real-time alert feature override boost ── */
  ...
  /* ── Community report urgency boost ── */
  ...
  /* incidentSeverity: 0‒1 from real DB incidents in this region */
  const raw = (
    baseScore           * 0.30 +
    incidentSeverity    * 0.20 +
    reportFreq[topThreat] * 0.12 +
    rtBoost             * 0.18 +   // Real-time GBIF/GFW/satellite boost
    commBoost           * 0.10 +   // Community report urgency signal
    (season - 0.5)      * 0.06 +
    (time - 1.0)        * 0.03 +
    (lunar - 1.0)       * 0.01
  );
...
/* ═══════════════════════════════════════════════════
   refreshAllPredictions()
   Pulls live MongoDB data and recomputes all zones.
   PRIMARY: Python sklearn RF via mlBridge
   FALLBACK: JS computation if Python is offline
═══════════════════════════════════════════════════ */
async function refreshAllPredictions() {
  try {
    ...
    /* ── 1. Aggregate reports per region × type (last 30 days) ── */
    ...
    /* ── 1b. Aggregate urgent community reports (last 7 days) ── */
    ...
    /* Build lookup: regionLower → { threatType: normalizedFreq } */
    ...
    /* ── 2. Aggregate incident severity per region (last 30 days) ── */
    ...
    /* ── 3. Check if Python ML is online ── */
    ...
    /* ── 4. Compute predictions for every zone ── */
    const predictions = await Promise.all(ZONES.map(async (zone) => {
      ...
      // JS enrichment gives us: threat_type, prediction text, factors narrative
      // Now includes real-time alert boosts + community report signals
      const jsResult = computePrediction(zone, reportFreq, incSev, communitySignals);

      if (!pythonOnline) {
        // Purely JS — keep as-is
        return { ...jsResult, ml_model: 'JS Random Forest (fallback)', source: 'js_fallback' };
      }

      try {
        // Pass DB-derived features to Python for the core risk_score
        const pyFeatures = {
        ...
        // Blend: Python score is authoritative; keep JS threat/prediction metadata
        return {
          ...jsResult,
        ...
        // Per-zone Python failure → use JS result for this zone
    ...
    /* Fall back to temporal-only JS predictions if DB unavailable */
...
/* ── Single-zone predict (for on-demand API calls) ── */
